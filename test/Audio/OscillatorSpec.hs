...
tests =
    testGroup
        ...
        , testCase "sawtooth stays in roughly [-1,1]" $
            -- PolyBLEP can overshoot slightly near the discontinuity; allow 0.05 slack.
            assertBool "in range" (all (\x -> x >= -1.05 && x <= 1.05) (samples Sawtooth))
        , testCase "square stays in roughly [-1,1]" $
            assertBool "in range" (all (\x -> x >= -1.05 && x <= 1.05) (samples Square))
        ]
